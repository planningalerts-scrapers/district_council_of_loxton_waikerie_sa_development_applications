...
async function main() {
    ...
    // Retrieve the main page (and establish a cookie jar).
    ...
    let now = moment();
    getResults(now.subtract(1, "months"), now);

    let monthCount = now.year() * 12 + now.month() - (2012 * 12 + 1);  // first recorded development application is in 2012
    let randomMonth = getRandom(1, monthCount + 1)
    getResults(parse(now.subtract(randomMonth + 1, <moment.unitOfTime.DurationConstructor> "months"), now.subtract(randomMonth, <moment.unitOfTime.DurationConstructor> "months"));
